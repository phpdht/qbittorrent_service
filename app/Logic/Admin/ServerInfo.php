...
class ServerInfo
{
    ...
    function __construct()
    {
        ...
        $this->sysInfo        = $this->GetsysInfo();        //var_dump($this->sysInfo);
    ...
    { //linux系统探测
        ...
        $res['uptime'] = $days . "天" . $hours . "小时" . $min . "分钟";        //内存
    ...
    { //格试化内存显示单位
        ...
        $tmp = array(
            'memTotal', 'memUsed', 'memFree', 'memPercent', 'memCached', 'memRealPercent', 'swapTotal', 'swapUsed', 'swapFree', 'swapPercent'
        );
        foreach ($mem as $k => $v) {
            if (!strpos($k, 'Percent')) {
                $v = $v < 1024 ? $v . ' M' : $v . ' G';
